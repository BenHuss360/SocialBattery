export default function DashboardLoading() {
  return (
    <main className="min-h-screen p-6">
      <div className="max-w-md mx-auto">
        {/* Header skeleton */}
        <div className="flex justify-between items-center mb-8">
          <div className="h-7 w-32 bg-card rounded animate-pulse" />
          <div className="flex items-center gap-4">
            <div className="h-4 w-16 bg-card rounded animate-pulse" />
            <div className="h-4 w-16 bg-card rounded animate-pulse" />
          </div>
        </div>

        {/* Battery skeleton */}
        <div className="flex justify-center mb-8">
          <div className="w-24 h-40 bg-card rounded-lg animate-pulse" />
        </div>

        {/* Status section skeleton */}
        <div className="bg-card rounded-xl border border-border p-6 mb-6">
          <div className="h-5 w-16 bg-background rounded animate-pulse mb-4" />
          <div className="flex flex-wrap gap-2 mb-4">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-10 w-24 bg-background rounded-full animate-pulse"
              />
            ))}
          </div>
          <div className="h-10 w-full bg-background rounded-lg animate-pulse" />
        </div>

        {/* Share section skeleton */}
        <div className="bg-card rounded-xl border border-border p-6 mb-6">
          <div className="h-5 w-36 bg-background rounded animate-pulse mb-4" />
          <div className="flex items-center gap-2 mb-4">
            <div className="flex-1 h-10 bg-background rounded-lg animate-pulse" />
            <div className="h-10 w-16 bg-background rounded-lg animate-pulse" />
          </div>
          <div className="flex gap-2">
            <div className="flex-1 h-10 bg-background rounded-lg animate-pulse" />
            <div className="flex-1 h-10 bg-background rounded-lg animate-pulse" />
          </div>
        </div>

        {/* Save indicator skeleton */}
        <div className="flex justify-center">
          <div className="h-4 w-40 bg-card rounded animate-pulse" />
        </div>
      </div>
    </main>
  );
}
